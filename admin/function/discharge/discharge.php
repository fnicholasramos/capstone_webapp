...
if (isset($_GET['id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $insert_sql = "INSERT INTO discharge (patient_name, iv_fluid, admission_date, admission_time, discharge_date, discharge_time, ivf_no)
                       VALUES ('{$row['patient_name']}', '{$row['iv_fluid_name']}', '{$row['date_started']}', 
                                '{$row['time_started']}', '$current_date', '$current_time_now', '{$row['ivf_no']}')";
